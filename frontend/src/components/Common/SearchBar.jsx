import { useState } from "react";
import { HiMagnifyingGlass, HiMiniXMark } from "react-icons/hi2";

const SearchBar = () => {
  const [searchTerm, setSearchTerm] = useState("");
  const [isOpen, setIsOpen] = useState(false);

  const handleSearchToggle = () => {
    setIsOpen(!isOpen);
  };

  const handleSearch = (e) =>{
    e.preventDefault()
    console.log("Search Term :: ", searchTerm);
    setSearchTerm('')
    setIsOpen(false)
  }

  return (
    <div
      className={`flex w-full flex justify-center items-center transition-all duration-300 ${
        isOpen ? "absolute w-full bg-white h-24 z-50 top-0 left-0" : "w-auto"
      }`}
    >
      {isOpen ? (
        <form onSubmit={handleSearch} className="relative w-full flex justify-center items-center">
          <div className="relative w-1/2">
            <input
              type="text"
              placeholder="Search"
              value={searchTerm}
              onChange={(e)=>setSearchTerm(e.target.value)}
              className="bg-gray-100 px-4 py-2 focus:outline-none w-full pl-2 pr-12 placeholder:text-gray-700 rounded-lg"
            />
            {/* Search icon */}
            <button type="submit" className="absolute top-1/2 right-2 transform -translate-y-1/2 text-gray-600 hover:text-gray-800">
              <HiMagnifyingGlass className="h-6 w-6 text-gray-700" />
            </button>
          </div>
            {/* Close Button */}
          <button type="button" onClick={handleSearchToggle}  className="absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-600 hover:text-gray-800">
                <HiMiniXMark className="h-6 w-6"/>
            </button>
        </form>
      ) : (
        <button onClick={handleSearchToggle}>
          <HiMagnifyingGlass className="h-6 w-6 text-gray-700" />
        </button>
      )}
    </div>
  );
};

export default SearchBar;
